...
Select top 2 * from salaries
Select top 2 * from school_details
Select top 2 * from schools
...
with school_CTE as (
                            Select Floor(yearID/10) * 10 as decade, Count(distinct schoolID) as total_schools
                            from schools
                            Group by yearID)

Select decade, sum(total_schools) as
total_schools  from school_CTE
Group by decade
order by decade
...
Select top 5 * from school_details
Select top 5 * from schools;
...
with school_CTE as (
         ...
         school_CTE2 as (
                            Select schoolID, Count(playerID) as total_players
                            from school_CTE
                            Group by schoolID)

Select Top 5 * 
from school_CTE2 sc2
Join school_details sd 
        on (sc2.schoolID = sd.schoolID)
order by total_players Desc


-- 4. For each decade, what were the names of the top 3 schools that produced the most players?
Select yearID, distinct (schoolID), Distinct(playerID) 
from schools
